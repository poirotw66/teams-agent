locals {
  ...
  portal_image = var.portal_image != "" ? var.portal_image : (
    var.allow_latest_image_tags
    ? "${local.artifact_registry_path}/${var.portal_service_name}:latest"
    : null
  )
  ...
  backoffice_env = {
    ...
    KNOWLEDGE_PORTAL_URL_CONFIGURED            = tostring(local.deploy_cloud_run || var.knowledge_portal_public_url != "")
    KNOWLEDGE_PORTAL_INTERNAL_URL              = local.deploy_cloud_run ? google_cloud_run_v2_service.portal[0].uri : (var.knowledge_portal_internal_url != "" ? var.knowledge_portal_internal_url : var.knowledge_portal_public_url)
    AI_OPS_KNOWLEDGE_INTERNAL_URL              = local.deploy_cloud_run ? google_cloud_run_v2_service.portal[0].uri : (var.knowledge_portal_internal_url != "" ? var.knowledge_portal_internal_url : var.knowledge_portal_public_url)
