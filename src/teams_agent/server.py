...
(`/healthz`, `/readyz`, the signed `/rag-assets/` image endpoint, and the
source viewer / SSO routes) is registered here on the same FastAPI instance,
so a single uvicorn server serves both.
