...
Production formal acceptance remains a separate gate and continues to require v2 externally verified evidence when production governance invokes it.

**Non-technical walkthrough (Traditional Chinese):** `docs/ai-ops-signoff-walkthrough-zh.md`

### 1. BU evidence — taxonomy and metrics (`bu-taxonomy-metrics`)
...
Confirm definitions match operational needs and include the result in the administrator review evidence.

### 2. IT evidence — Terraform (`it-terraform`)
...
### 3. Security/Legal evidence — masking, retention, export (`security-masking-retention`)
...
### 4. Knowledge Portal governance evidence (`knowledge-portal-governance`)
...
After reviewing all evidence, record the one administrator decision:
...
python scripts/ops_signoff_approve.py \
  ...
  --by "Justin" \
  --notes "SYSTEM_ADMIN reviewed Phase 1 technical and governance evidence."
```

Validate and close the Phase 1 milestone:
...
Production formal acceptance, when invoked, uses a separate v2 evidence bundle with one externally verified `SYSTEM_ADMIN` approval.
