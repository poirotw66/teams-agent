...
PORTAL_IMAGE="${REGISTRY}/teams-knowledge-portal:latest"
...
PORTAL_TOKEN="$(gcloud secrets versions access latest \
  --secret=teams-knowledge-portal-token --project="${PROJECT_ID}")"
...
gcloud run deploy "${PORTAL_SERVICE}" \
  ...
  --max-instances=3 \
  --timeout=600 \
  --set-env-vars="KNOWLEDGE_PORTAL_HOST=0.0.0.0,KNOWLEDGE_PORTAL_PORT=8080,KNOWLEDGE_PORTAL_DATA_DIR=/app/data,KNOWLEDGE_PORTAL_AUTH_MODE=HEADER,KNOWLEDGE_PORTAL_RELAXED_WORKFLOW=true,KNOWLEDGE_PORTAL_DEMO_MODE=true,KNOWLEDGE_PORTAL_REPOSITORY_MODE=FIRESTORE,GCP_PROJECT_ID=${PROJECT_ID},KNOWLEDGE_PORTAL_ARTIFACT_STORAGE_BACKEND=GCS,AI_OPS_ARTIFACT_STORAGE_BACKEND=GCS,KNOWLEDGE_PORTAL_ARTIFACT_GCS_BUCKET=${ARTIFACT_GCS_BUCKET},AI_OPS_ARTIFACT_GCS_BUCKET=${ARTIFACT_GCS_BUCKET},KNOWLEDGE_PORTAL_SOURCE_STORE_MODE=FIRESTORE,AI_OPS_SOURCE_STORE_MODE=FIRESTORE,KNOWLEDGE_PORTAL_DEFAULT_TENANT_ID=default,KNOWLEDGE_PORTAL_TOKEN=${PORTAL_TOKEN},KNOWLEDGE_PORTAL_AGENT_API_URL=${AGENT_URL},KNOWLEDGE_PORTAL_PUBLIC_URL=https://placeholder.invalid,TEAMS_ADAPTER_URL=${ADAPTER_URL}"
...
if [[ -n "${BACKOFFICE_SA}" ]]; then
  ...
  gcloud run services update "${BACKOFFICE_SERVICE}" \
    --region="${REGION}" --project="${PROJECT_ID}" \
    --update-env-vars="KNOWLEDGE_PORTAL_PUBLIC_URL=${PORTAL_URL},KNOWLEDGE_PORTAL_INTERNAL_URL=${PORTAL_URL},KNOWLEDGE_PORTAL_TOKEN=${PORTAL_TOKEN}" >/dev/null || true
fi
