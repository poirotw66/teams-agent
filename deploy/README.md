...
Set `RELEASE_COMPONENTS=converter` to build and update only the PDF converter.
The converter build pins its upstream source revision and maintains a separate
Artifact Registry build-cache tag.
...
| 服務實例 | 角色與配置 | 部署方式 |
|---|---|---|
| **Backoffice API** (`teams-ai-ops-backoffice`, override with `GCP_BACKOFFICE_API_SERVICE`) | 提供 HTTP API 與前端介面，設定 `AI_OPS_WORKERS_ENABLED=false`，以 `concurrency=20`、`min=0`、`max=10` 水平擴展以控制閒置成本；縮放至零後的首次請求仍可能承擔冷啟動。事件查詢快取預設 120 秒，可用 `AI_OPS_QUERY_CACHE_TTL_SECONDS` 調整。原檔請設定獨立的 `AI_OPS_ARTIFACT_GCS_BUCKET`（勿與 export bucket 共用）。 | `uvicorn ai_ops_backoffice.main:app` |
