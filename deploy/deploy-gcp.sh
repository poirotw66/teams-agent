...
KNOWLEDGE_RELEASE_PREFIX="${KNOWLEDGE_RELEASE_GCS_PREFIX:-knowledge-releases}"
KNOWLEDGE_RELEASE_TENANT_ID="${KNOWLEDGE_RELEASE_TENANT_ID:-default}"
KNOWLEDGE_RELEASE_STORE_MODE="${KNOWLEDGE_RELEASE_STORE_MODE:-GCS}"
...
# Operational events must land in the same Firestore the AI Ops backoffice reads.
AGENT_DEPLOYMENT_ENV="${AGENT_DEPLOYMENT_ENV:-poc}"
KNOWLEDGE_RELEASE_MODE="${KNOWLEDGE_RELEASE_MODE:-AUTO}"
KNOWLEDGE_RELEASE_DIR="${KNOWLEDGE_RELEASE_DIR:-/app/data/releases}"
KNOWLEDGE_ACTIVE_RELEASE_ID="${KNOWLEDGE_ACTIVE_RELEASE_ID:-release-19072ac9a1e2}"
KNOWLEDGE_RELEASE_REQUIRE_MANIFEST="${KNOWLEDGE_RELEASE_REQUIRE_MANIFEST:-true}"
KNOWLEDGE_RELEASE_REQUIRE_VECTORS="${KNOWLEDGE_RELEASE_REQUIRE_VECTORS:-true}"
OPS_EVENTS_COLLECTION="${OPS_EVENTS_COLLECTION:-operational_events}"
OPS_AUDIT_COLLECTION="${OPS_AUDIT_COLLECTION:-audit_events}"
OPS_BIGQUERY_DATASET="${OPS_BIGQUERY_DATASET:-ai_ops_analytics}"
OPS_BIGQUERY_TABLE="${OPS_BIGQUERY_TABLE:-operational_events}"
# Keep Firestore as the source of truth for the backoffice. BigQuery is optional
# analytics; enable only when the agent SA has bigquery.jobUser + dataEditor.
OPS_BIGQUERY_ENABLED="${OPS_BIGQUERY_ENABLED:-false}"
...
require_value "${KNOWLEDGE_ACTIVE_RELEASE_ID}" "KNOWLEDGE_ACTIVE_RELEASE_ID"
require_value "${KNOWLEDGE_RELEASE_BUCKET}" "KNOWLEDGE_RELEASE_GCS_BUCKET"
if [[ "${KNOWLEDGE_RELEASE_STORE_MODE}" != "GCS" ]]; then
  fail "雲端部署必須使用 KNOWLEDGE_RELEASE_STORE_MODE=GCS，本機 FILE 索引不會打進映像"
fi
...
if ! gcloud storage objects describe "${TENANT_RELEASE_INDEX}" \
  ...
  && ! gcloud storage objects describe "${LEGACY_RELEASE_INDEX}" \
    --project="${PROJECT_ID}" >/dev/null 2>&1; then
  log "警告：GCS 尚無 ${KNOWLEDGE_ACTIVE_RELEASE_ID} 的 index；映像仍可部署，Agent /readyz 要等 Portal 發布此 release"
fi

log "確認 Viewer membership GCS bucket（跨 Adapter 實例共用）"
ensure_private_bucket "${VIEWER_MEMBERSHIP_BUCKET}"
grant_bucket_role \
  "${VIEWER_MEMBERSHIP_BUCKET}" \
  "serviceAccount:${ADAPTER_SA}" \
