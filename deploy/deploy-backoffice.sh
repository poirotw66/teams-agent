...
SOURCE_DELEGATION_SECRET="${AI_OPS_SOURCE_DELEGATION_SECRET:-${RAG_ASSET_SIGNING_KEY:-}}"
...
gcloud run deploy "${BACKOFFICE_API_SERVICE}" \
  ...
  --set-env-vars="AI_OPS_WORKERS_ENABLED=false,AI_OPS_BACKOFFICE_PORT=8080,${SHARED_ENV_VARS}" \
  --port=8080 \
  --cpu=1 \
  --memory=1Gi \
  --concurrency=20 \
...
gcloud run deploy "${BACKOFFICE_WORKER_SERVICE}" \
  ...
  --set-env-vars="AI_OPS_WORKERS_ENABLED=true,AI_OPS_WORKER_PORT=8080,${SHARED_ENV_VARS}" \
  --port=8080 \
  --cpu=1 \
  --memory=1Gi \
  --min-instances=1 \
