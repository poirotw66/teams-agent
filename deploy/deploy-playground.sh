...
gcloud run deploy "${PLAYGROUND_SERVICE}" \
  ...
  --min=0 \
  --max=1 \
  --set-env-vars="BOT_ENDPOINT=${ADAPTER_URL}/api/messages,DEFAULT_CHANNEL_ID=msteams,AUTH_CLIENT_ID=${BOT_CLIENT_ID},AUTH_TENANT_ID=${BOT_TENANT_ID},PLAYGROUND_PUBLIC_BASE_URL=${PLAYGROUND_PUBLIC_BASE_URL},KNOWLEDGE_CONTROL_URL=${AGENT_URL}/admin/knowledge-backend,KNOWLEDGE_CONTROL_AUTH_MODE=google_id_token,KNOWLEDGE_CONTROL_AUDIENCE=${AGENT_URL}" \
