...
| :--- | :--- | :--- | :--- |
| **AI 營運後台 (AI Ops Console)** | `8092` | 公開／營運 | **主要統一營運 BFF** — React `/console-v2` 為預設產品 UI（`/` 導向）；legacy shell 預設關閉（需 `BACKOFFICE_LEGACY_SHELL_ENABLED`） |
...
- **AI 資訊客服營運後台（Port 8092；統一營運入口 & BFF）**：
  - `GET /console-v2/*`：React Console（唯一 active 產品 UI）
  - `GET /`：導向 `/console-v2/dashboard`（legacy 不在產品路徑）
  - `GET /legacy`：僅在 `BACKOFFICE_LEGACY_SHELL_ENABLED=true` 時提供緊急 legacy shell（否則同樣導向 `/console-v2/dashboard`；擁有者見 `docs/ai-ops-route-ledger.md`）
...
跨 domain 配線在 `composition/`，共用 port 在 `platform_kernel/`。Domain 之間不可反向 import；CI 以 `scripts/check_architecture.py` 與 Adapter↔Agent 欄位相容檢查 `scripts/check_wire_contracts.py` 驗證。路由擁有者見 [`docs/ai-ops-route-ledger.md`](docs/ai-ops-route-ledger.md)（30/30 v2）。超標殘件與複審日期見 [`docs/architecture/oversized-waivers.md`](docs/architecture/oversized-waivers.md)。
