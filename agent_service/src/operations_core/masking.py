...
from operations_core.masking_rules import MaskingRulePack, apply_masking_pack, resolve_masking_pack
...
class ActiveMaskingPolicy(Protocol):
    """Minimal policy view required by mask_text."""
...
class _BaselineMaskingPolicy:
    policy_version: str
    pack: MaskingRulePack


_active_masking_policy_provider: Callable[[], ActiveMaskingPolicy] | None = None


def register_active_masking_policy_provider(
    provider: Callable[[], ActiveMaskingPolicy] | None,
) -> None:
    """Allow Agent policy_runtime to supply request-scoped masking policy.

    operations_core must not import agent_service; the Agent package registers
    its resolver during policy_runtime import.
    """
    global _active_masking_policy_provider
    _active_masking_policy_provider = provider


def _resolve_active_masking_policy() -> ActiveMaskingPolicy:
    if _active_masking_policy_provider is not None:
        return _active_masking_policy_provider()
    pack = resolve_masking_pack(MASKING_POLICY_VERSION)
    return _BaselineMaskingPolicy(policy_version=pack.policy_version, pack=pack)
...
def mask_text(text: str, *, reveal: bool = False) -> MaskingResult:
    policy = _resolve_active_masking_policy()
    pack = policy.pack
