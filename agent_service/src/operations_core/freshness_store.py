...
class FreshnessStore:
                        ...
                        if not committed:
                            with self.lock:
                                ...
                                if cur is None or remote_dt > cur:
                                    self.last_successful_sync[key] = remote_dt
                            return
                        return
                    ...
                    if at < existing_dt:
                        with self.lock:
                            ...
                            if cur is None or existing_dt > cur:
                                self.last_successful_sync[key] = existing_dt
                        return
                except Exception:
                    pass
                    ...
                    try:
                        ...
                        if not committed:
                            with self.lock:
                                cur = self.worker_heartbeats.get(worker_id)
                                if cur is None or remote_dt > cur:
                                    self.worker_heartbeats[worker_id] = remote_dt
                            return
                        return
                    except Exception as txn_exc:
                        if attempt < max_retries - 1:
                ...
                try:
                    ...
                    if at < existing_dt:
                        with self.lock:
                            cur = self.worker_heartbeats.get(worker_id)
                            if cur is None or existing_dt > cur:
                                self.worker_heartbeats[worker_id] = existing_dt
                        return
                except Exception:
                    pass
        elif hasattr(doc_ref, "coll") and hasattr(doc_ref.coll, "store"):
                    ...
                    for k, v in watermarks.items():
                        if isinstance(v, str) and k not in ("heartbeats", "backlogs"):
                            try:
                                ...
                                existing = self.last_successful_sync.get(k)
                                if existing is None or dt > existing:
                                    self.last_successful_sync[k] = dt
                            except Exception:
                                pass
                    ...
                    for wid, ts_str in heartbeats.items():
                        if isinstance(ts_str, str):
                            try:
                                ...
                                existing_hb = self.worker_heartbeats.get(wid)
                                if existing_hb is None or hb_dt > existing_hb:
                                    self.worker_heartbeats[wid] = hb_dt
                            except Exception:
                                pass
    ...
    def save_persistent(self) -> None:
        ...
        try:
            ...
            payload = {
                "watermarks": {k: v.isoformat() for k, v in self.last_successful_sync.items()},
                "heartbeats": {k: v.isoformat() for k, v in self.worker_heartbeats.items()},
                "backlogs": {
                    k: {
                        "count": v.get("count", 0),
                        "oldest_pending_at": (
