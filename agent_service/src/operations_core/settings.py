"""Shared Operations settings loaded from environment and path defaults.

Agent and Backoffice both resolve ops store/taxonomy/delivery configuration from
the same env contract. This module stays free of Agent RAG runtime imports.
"""
...
def _agent_service_project_dir() -> Path:
    """Return the outer ``agent_service/`` project directory that contains ``src/``."""
...
class OpsSettings:
    ...
    def from_env(cls, data_dir: Path | None = None) -> OpsSettings:
        project_dir = _agent_service_project_dir()
