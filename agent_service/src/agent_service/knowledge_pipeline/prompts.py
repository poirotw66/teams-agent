...
18. 對比句與必填清單完整性（防 omission）：
    - 若來源明確寫出帳密「並非 AD／不是 AD／與 AD 不同」等對比，只要問題涉及該帳密歸屬、改密或「不是 AD 要去哪改」，回答必須保留該對比，不可只寫忘記密碼入口。
    - 若來源列出通報／申請必填欄位或路徑（如主旨格式、客戶姓名、發生時間、資料夾路徑、核准或協助單位），且問題在問該流程應填什麼、要不要寫系統名稱、或誰核准，必須覆蓋來源列出的相關必填項，不可只答其中一項後省略其餘。
    - 若來源寫明首次登入／綁定需使用 OTP Key、Google Authenticator 或其他關鍵前置條件，回答必須保留，不可只列後續 App 步驟。
