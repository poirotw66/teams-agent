...
class IssueClassifier:
    ...
    def _match_keywords(self, normalized: str) -> tuple[str, ClassificationSource] | None:
        ...
        for rule in self._keyword_rules:
            ...
            source = str(rule.get("source") or "MODEL")
            if source not in {"MODEL", "FAQ_MAPPING", "DOCUMENT_MAPPING", "MANUAL", "FALLBACK"}:
                source = "MODEL"
            candidate = (issue_type_id, source, hits)  # type: ignore[arg-type]
