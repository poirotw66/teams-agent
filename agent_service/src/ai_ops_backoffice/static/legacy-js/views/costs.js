...
function appendCostSection(parent, title, node, note = "") {
  const section = el("section", "ops-block");
  section.append(el("h3", "ov-panel-title", title));
  if (note) section.append(el("p", "ov-panel-note", note));
  section.append(node);
  parent.append(section);
}

function costStatusBadge(item, totalTokens) {
  if (item.costStatus === "ZERO_COST" || (item.estimatedCostUsd === 0 && totalTokens === 0)) {
    return badge("零成本", "neutral");
  }
  if (item.costStatus === "UNKNOWN" || item.estimatedCostUsd == null) {
    return badge("未知成本", "warning");
  }
  return badge("已估算", "success");
}
...
export async function renderCosts(state = { preset: "30d" }) {
  ...
  try {
    ...
    toolbar.append(createPeriodControls(period, reloadWithFilters));

    const filterBar = el("div", "filter-bar");
    const applyModel = el("button", "", "套用");
    ...
    if (modelFilter) {
      panel.append(el("p", "ov-status", `目前只看 ${modelFilter}`));
    }

    const unknownCost = data.missingCostEventCount ?? 0;
    panel.append(
      kpiStrip([
        { label: "預估費用（美元）", value: Number(data.totalEstimatedCostUsd ?? 0).toFixed(4) },
        {
          label: "預估費用（新台幣）",
          value: data.totalEstimatedCostTwd != null ? Number(data.totalEstimatedCostTwd).toFixed(2) : "-",
        },
        { label: "零成本事件", value: String(data.zeroCostEventCount ?? 0) },
        {
          label: "未知成本",
          value: String(unknownCost),
          attention: unknownCost > 0,
        },
      ]),
    );

    const usage = el("p", "ov-status");
    const usageParts = [
      `輸入 ${(data.inputTokens ?? 0).toLocaleString("zh-TW")}`,
      `輸出 ${(data.outputTokens ?? 0).toLocaleString("zh-TW")}`,
      `合計 ${((data.inputTokens ?? 0) + (data.outputTokens ?? 0)).toLocaleString("zh-TW")}`,
    ];
    if (data.usdTwdExchangeRate) usageParts.push(`匯率 ${data.usdTwdExchangeRate}`);
    usage.textContent = usageParts.join("，");
    panel.append(usage);

    const modelTable = el("table");
    modelTable.innerHTML =
      "<thead><tr><th>模型</th><th>成本狀態</th><th>輸入量</th><th>輸出量</th><th>合計</th><th>事件</th><th>預估美元</th><th>輸入單價</th><th>輸出單價</th></tr></thead>";
    ...
    for (const item of data.byModel || []) {
      ...
      statusCell.append(costStatusBadge(item, totalTokens));
    ...
    appendCostSection(panel, "依模型", modelScroll);

    const more = el("details", "ops-more");
    more.append(el("summary", "", "日期、處理方式與費率明細"));

    const ratesTable = el("table");
    ratesTable.innerHTML =
      "<thead><tr><th>模型</th><th>輸入單價（美元 / 百萬）</th><th>輸出單價（美元 / 百萬）</th><th>定價版本</th></tr></thead>";
    ...
    table.innerHTML = "<thead><tr><th>日期</th><th>預估美元</th></tr></thead>";
    const body = el("tbody");
    ...
    appendCostSection(more, "依日期", dateScroll);

    const routeTable = el("table");
    routeTable.innerHTML = "<thead><tr><th>處理方式</th><th>預估美元</th></tr></thead>";
    const routeBody = el("tbody");
    for (const item of data.byRoute || []) {
      const row = el("tr");
      row.append(el("td", "", labelRoute(item.route)));
      row.append(el("td", "", String(item.estimatedCostUsd)));
    ...
    appendCostSection(more, "依處理方式", routeScroll);

    const issueTable = el("table");
    issueTable.innerHTML =
      "<thead><tr><th>問題代碼</th><th>問題名稱</th><th>預估美元</th></tr></thead>";
    ...
    appendCostSection(more, "依問題", issueScroll);

    for (const [heading, items, key] of [
      ["供應商", data.byProvider, "provider"],
      ["元件", data.byComponent, "component"],
      ["知識後端", data.byBackend, "backend"],
    ]) {
      const dimensionTable = el("table");
      dimensionTable.innerHTML = `<thead><tr><th>${heading}</th><th>事件</th><th>預估美元</th></tr></thead>`;
      ...
      appendCostSection(more, `依${heading}`, dimScroll);
    }
    if (data.embeddingTokens != null || data.toolContextTokens != null || data.pricingVersion) {
      const extra = el("p", "ov-panel-note");
      const parts = [];
      if (data.embeddingTokens != null) parts.push(`嵌入 ${(data.embeddingTokens ?? 0).toLocaleString("zh-TW")}`);
      if (data.toolContextTokens != null) parts.push(`工具上下文 ${(data.toolContextTokens ?? 0).toLocaleString("zh-TW")}`);
      if (data.pricingVersion) parts.push(`定價版本 ${data.pricingVersion}`);
